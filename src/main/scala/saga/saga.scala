...
object Saga {
  ...
  // syntax
  implicit class SagaSintax[F[_]: Applicative, A, E](saga: Saga[F, A, E]) {
  ...
  private def transact_[F[_]: Monad, A, E](saga: Saga[F, A, E]): F[Either[E, A]] = {
    ...
    def anify[X, Y, Z](f: X => Saga[F, Y, Z]) = f.asInstanceOf[Any => Saga[F, Any, Any]]

    def recover(eff: F[Either[Any, Any]], handles: List[Any => F[Any]]): F[Either[Any, Any]] = {
      if (handles.isEmpty) eff else
      for {
        errorOrResult <- eff
    ...
    def go(
      ...
      case Bind(s1, k1) =>
        go(rollback, M.pure(s1), anify(k1) :: cont)
    }
